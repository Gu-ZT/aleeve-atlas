...
public record ArrowMarkerRenderState(
    ...
    int color, GpuBufferSlice arrowMarkerUniform,
    @Nullable ScreenRectangle scissorArea, @Nullable ScreenRectangle bounds
) implements LibGuiElementRenderState {

    public ArrowMarkerRenderState(Matrix3x2f pose, Vector2f p0, Vector2f p1, Vector2f p2, Vector2f p3,
                                   int color, GpuBufferSlice arrowMarkerUniform, @Nullable ScreenRectangle scissorArea) {
        ...
        this(pose, p0, p1, p2, p3, color, arrowMarkerUniform, scissorArea,
    ...
    @Override public RenderPipeline pipeline() { return ModRenders.ARROW_MARKER_PIPELINE; }
    @Override public TextureSetup textureSetup() { return TextureSetup.noTexture(); }
    @Override public Map<String, GpuBufferSlice> bufferSlices() { return Map.of("ArrowMarkerUniform", arrowMarkerUniform()); }

    @Nullable
    public static GpuBufferSlice createArrowMarkerUniform(Vector2fc clipCenter, Vector2fc clipHalfSize, float clipRadius,
                                                           float clipMode, Vector2fc markerCenter, float markerRadius, float arrowAngle) {
        ...
        return AleeveAtlasClient.getModDynamicUniforms().getArrowMarkerUbo().writeUniform(
            new ModDynamicUniforms.ArrowMarkerUniform(clipCenter, clipHalfSize, clipRadius, clipMode, markerCenter, markerRadius, arrowAngle));
    }
}

...
public final class AtlasRadar {
    ...
    public static void render(Minecraft minecraft, GuiGraphicsExtractor graphics, int mapX, int mapY, int mapSizePx, float rotationDeg) {
        ...
        AABB qb = new AABB(playerX - scanRange, minecraft.player.getY() - scanRange, playerZ - scanRange,
        ...
        for (Entity e : minecraft.level.getEntities(minecraft.player, qb, Entity::isAlive)) {
            ...
            int color = markerColor(e); if (color == 0) continue;
            double mlx = dx*cos + dz*sin, mlz = -dx*sin + dz*cos;
            markers.add(new RadarMarker(cx + (mlx/displayRange)*radiusPx, cy + (mlz/displayRange)*radiusPx, color, dx*dx+dz*dz, e.getId()));
        }
        ...
        for (int i = 0; i < limit; i++) {
            ...
            if (u == null) { int r = Math.round(ENTITY_MARKER_RADIUS); graphics.fill((int)m.px-r, (int)m.py-r, (int)m.px+r+1, (int)m.py+r+1, m.color); continue; }
            float pad = ENTITY_MARKER_RADIUS + 1f;
            graphics.submitGuiElementRenderState(new MarkerRenderState(graphics.pose(),
